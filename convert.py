...
def read_annotations(filename):
    ...
    for line in lines:
        (xmin, ymin, xmax, ymax) = list(map(float, line.split()))
...
def write_clean_annos(filename, size, annos, filestream, image_file):
    ...
    for i in range(len(annos)):
        ...
        f.write(str(cls_id) + " " + " " + os.path.join([str(a) for a in bb]) + '\n')
...
def process_video(images_dir, annos_dir, clean_annos_dir, fileliststream):
    ...
    imagefiles = [f for f in os.listdir(images_dir) if os.listdir(os.path.join(images_dir, f))]

    annotations = [f for f in os.listdir(annos_dir) if os.listdir(os.path.join(annos_dir, f))]
    annotations = np.array(annotations)
    annotations = np.sort(annotations)

    # filtering '*.jpg' files
    imagefiles = [image_file for image_file in imagefiles if
                  image_file[image_file.rfind('.') + 1:] == 'jpg' or image_file[image_file.rfind('.') + 1:] == 'png']
    imagefiles = np.array(imagefiles)
    imagefiles = np.sort(imagefiles)

    for i in range(imagefiles.shape[0]):
        image_file = os.path.join(images_dir, imagefiles[i])
        im = cv2.imread(os.path.join(images_dir, imagefiles[i]))
        annos = read_annotations(os.path.join(annos_dir, annotations[i]))
        ...
        (h, w) = im.shape[:2]
        size = (w, h)
        write_clean_annos(os.path.join(clean_annos_dir, annotations[i]), size, annos, fileliststream, image_file)
        draw_annos(im, annos)
        cv2.imshow('image', im)
        cv2.waitKey(1000 / 12)
...
def get_immediate_subdirectories(a_dir):
    return [name for name in os.os.listdir(a_dir)
            if os.path.isdir(os.path.os.path.join(a_dir, name))]


def main():
    parser = argparse.ArgumentParser()

    parser.add_argument('-images_root', default='E:\\dataset\\fire_dataset\\Train\\images',
                        # This dir name I cannot change cuz it will cause confusion
    ...
    for group in groups:
        os.makedirs(os.path.join(args.yolo_annos_root, group))
        print("Passing %s" % ('-images %s -annos %s -yolo_annos %s' % (
            os.path.join(args.images_root, group), os.path.join(args.annos_root, group), os.path.join(args.yolo_annos_root, group))))
        
        process_video(os.path.join(args.images_root, group), os.path.join(args.annos_root, group), os.path.join(args.yolo_annos_root, group),
                      fileliststream)
